...
beg=[1515283200+freq1*t for t in range(20)]
end=[1522540800+freq1*t for t in range(20)]

with open('/home/fbuonerba/codes/meta_data/new_coins.txt') as ff:
    coins=json.load(ff)

coo=['BTC','ETH','XRP','BCH','EOS','XLM','LTC','ADA','XMR','IOTA','TRX','ETC','DASH','NEO','XEM','BNB','XTZ','VET','ZEC','OMG','LSK','ZRX','QTUM','ONT','BTG','DCR','BCN','MKR','DOGE','BTS','ZIL','DGB','ICX','STEEM','NANO','AE','WAVES','BAT','SC','REP']
newcoins=[x for x in coins if x not in coo]
quotes=['USD','BTC']
...
pool.daemon=False
results=[pool.apply_async(cfl, args=(coin,quote,beg[t],end[t],freq,)) for coin in newcoins for quote in quotes for t in range(20)]
